                          ...
                          alpha = 0.5)
{
  ...
  if(is.null(fit_obj$Dn)) stop("for argument 'fit_obj', you should have 'method == chol' in function 'fit_rvfl'")
        ...
        if (method == "direct")
        {
            if (nlambda > 1)
            {
              for(i in 1:nlambda){
                ...
                # cat("check update", "\n")
                # print(update_factor%*%crossprod(scaled_augmented_newx))
                # cat("\n")
              ...
              # cat("check update", "\n")
              # print(update_factor%*%crossprod(scaled_augmented_newx)%*%(fit_obj$Sigma -
              #                                                             2*diag(ncol_Sigma)))
              # cat("\n")
            ...
            if (method == "direct")
            {
              ...
              if (nlambda > 1)
              {
                ...
                Dn <- lapply(1:nlambda, function (i)
                  chol2inv(chol(XTX + diag(x = fit_obj$lambda[i],
                                        nrow = ncol(XTX)))))
                fit_obj$Dn <- Dn
              } else {
                Dn <- chol2inv(chol(XTX + diag(x = fit_obj$lambda,
                                        nrow = ncol(XTX))))
                fit_obj$Dn <- Dn
              }
            }
